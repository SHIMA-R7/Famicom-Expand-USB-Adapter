...
**A0・A1はファミコン拡張端子(DA15)の2番・3番ピンとも共用**している。ファミコン側では未使用/未確定のピンだが、キーボード/ザッパーの4番・5番ピン共用と同じ考え方で、**SFCコントローラーとファミコンのキーボード/ザッパーは同時接続不可**として運用する。2番ピンは資料上「MIC?」と未確定のため、実害がないかテスターで導通・電圧を確認しておくこと推奨。
...
- 同様にDA15の2番・3番ピンはNanoのA0・A1(SFCコントローラーのLATCH/1P DATA)と共用しているため、**SFCコントローラーとファミコンのキーボード/ザッパーも同時接続不可**。
...
- DA15 2番ピン(「MIC?」表記)が本当に無信号か、テスターでの実機確認
