...
- SFCマウス: **完全に実機確認済み**(移動・左クリック・右クリック)。右ボタンは元のスイッチが導通不良だったためタクトスイッチに交換して解決。ファミコンベーシック用キーボードと同時使用しても問題なし
...
### SFCコントローラー/マウス対応(パッド・マウスとも実機確認済み)

- 1P/2Pとも毎スキャンごとに32bit読み、**マウス固有のシグネチャ(生データのbyte1下位4bitが常に`1110`固定)で自動判定**(標準パッドは切替スイッチ不要、ホットスワップにも対応)。
  当初は「先頭バイトが0x00ならマウス」で判定していたが、**標準パッドもボタン全解放時は先頭バイトが同じ値になり誤判定する**ことが実機検証で判明したため、シグネチャ判定に変更した
- 信号は全ビットがアクティブLowで伝送されるため、マウスのbyte1/2/3は反転してから解釈する。**L/Rボタンはbyte1の上位2bit(bit7=Right、bit6=Left)** — 実機の分解調査でこの位置を確定した(資料によってはbit0/1側と記載されており誤りだった)
...
- マウスは移動量(dx/dy)・L/Rボタンを`Mouse.move()`/`press()`/`release()`に変換。SFC純正の分解能(感度0で50カウント/インチ相当)は今どきのUSBマウスよりかなり粗いため、`code.py`の`SFC_MOUSE_GAIN_DESKTOP`(既定12倍)で底上げしている。1回のHIDレポートは±127までなので、ゲイン後の移動量が超える分は複数回のmove()に分割して送信(単純にクランプすると速い動きが軒並み同じ最大値に潰れてカクつくため)。好みに応じて調整可
- **エミュレータで使うときは等倍に切り替える必要がある。** RetroArch等でSFCマウス対応ソフト(マリオペイント等)を動かす場合、エミュレータ側が期待しているのは「SFCマウス実機の生のカウント」なので、底上げ済みの値を渡すと二重に補正されてカーソルが吹っ飛び、まともに操作できない。**左右ボタン同時長押し(1秒)**で`SFC_MOUSE_GAIN_DESKTOP`(12倍)と`SFC_MOUSE_GAIN_NATIVE`(等倍)を切り替えられる。切替後はLEDで通知(等倍=1個点灯、12倍=4個点灯)。PC側で何が動いているかはPicoから知りようがないため自動判別はできず、手動切替としている。なお切替中の1秒間は左クリック押しっぱなしとして送られるので、描画されない場所で操作すること
- マウスの更新頻度はキーボード走査の行間の隙間を使って約10倍に上げてある(キーボードの走査速度自体は変えていない)。それでも体感するカクつきは、SFCマウス側センサーの応答限界による可能性が高い
- 1P/2Pどちらのマウスも同じUSBマウスカーソルを共有する(OS側の制約、2台同時使用時は移動が混ざる)
- ファミコン側(キーボード/ザッパー)とSFC側は配線・処理とも独立しているため、モード切替スイッチに関係なく**常時同時動作**。ファミリーベーシック用キーボード+SFCマウスの同時使用も確認済み
...
- Nano側ループ末尾の待機を5ms→1msに短縮し、クリック/移動の応答性を上げた(キーボード走査自体は500us×18回≒9msかかるためこの短縮が主な効果)
- SFCボタンの読み取りにはキーボード側と同じデバウンス処理を追加済み。当初は未実装で、SELECT(右Shift)のチャタリングがWindowsの固定キー機能(Sticky Keys、Shift 5連打で起動)を誤爆させていた

パッド・マウス(移動/左クリック/右クリック)とも実機確認済み。右クリックは当初手元の個体のスイッチが導通不良だったが、タクトスイッチに交換して解決した。
...
ファームウェア実装・実機確認とも完了(パッド/マウスとも動作確認済み、詳細は下記「ファームウェア」参照)。
