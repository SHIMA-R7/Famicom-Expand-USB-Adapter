...
[`tools/zapper-test/index.html`](tools/zapper-test/index.html) — ブラウザで開くだけで動く射撃ゲーム。ザッパーの実機動作確認用。

**光線銃には位置を検出する機能がない**(光センサーとトリガースイッチだけ)ため、実機のダックハントと同じ方式で命中判定している:

1. トリガーを引くと画面を**全黒**にする(この間に受光したら外光を拾っている = 不正として棄却。実機と同じ仕組み)
2. 画面上の的を**1つずつ順番に白い四角で点灯**させる
3. センサーが受光した**タイミング**から、どの的を狙っていたかを特定する

実機は1フレーム(16.6ms)ずつ点灯させるが、こちらはUSB HID経由のぶん遅延が乗るため**既定150ms**に伸ばしてある(画面上のスライダーで調整可)。この方式ならフレーム精度に依存しないので、実機と違って**LCDでも動作する**。

- 画面下のランプでトリガー/センサーの入力をリアルタイム表示。**ゲームを始めなくても配線チェックだけに使える**
- 撃った履歴と「走査何番目で受光したか」がログに残るので、チャタリングや取りこぼしの確認にも使える
- 命中しない場合は点灯時間を長くする、画面を明るくする、部屋を暗くする

Pico側がザッパーモード(GP6のスイッチ)になっていないと反応しないので注意。
