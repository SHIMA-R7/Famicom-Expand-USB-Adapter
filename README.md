...
- マウスは移動量(dx/dy)・L/Rボタンを`Mouse.move()`/`press()`/`release()`に変換。SFC純正の分解能(感度0で50カウント/インチ相当)は今どきのUSBマウスよりかなり粗いため、`code.py`の`SFC_MOUSE_GAIN`(既定12倍)で底上げしている。1回のHIDレポートは±127までなので、ゲイン後の移動量が超える分は複数回のmove()に分割して送信(単純にクランプすると速い動きが軒並み同じ最大値に潰れてカクつくため)。好みに応じて調整可
